...
for (score in score.names) {
  ...
  aggregate(as.formula(paste0(score, ' ~ anchor.groups + Time')), FUN = function(x) mean(is.na(x)), data = dat, na.action = na.pass)
  ...
  # Fit MMRM
  mod.us1 <- glmmTMB(as.formula(paste0(score.mmrm, '~ anchor.groups + us(Time + 0 | USUBJID)')),
                  data=dat[dat$Time != 'Time_1', ],
                  REML = T,
                  dispformula=~0)

  emm <- emmeans::emmeans(mod.us1, ~ anchor.groups,
                            #data = getData(mod.us1),
                            mode = 'df.error')
   ...
   # mod.gls <- nlme::gls(as.formula(paste0(score.mmrm, '~ anchor.groups')),
   #                       dat = dat,
   ...
   #
   #
   #  emm <- emmeans::emmeans(mod.gls, ~ anchor.groups,
