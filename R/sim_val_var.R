...
#n.val <- n.val + 1 # ADD PGIS!!!!
#n.cat <- c(5, n.cat) # Add 3 or 5 category? PGIS anchor,
#cor.val.ref <- c(0.8, cor.val.ref) # Add PGIS - need correlation of 0.8 to yield R2 of 0.65
tt <- sort(unique(dat$Time))[1]
bl <- dat$Y_comp[dat$Time == tt ] # needs to have this correlation at baseline, not across all timepoints!
...
#colnames(cor.mat.ref) <- row.names(cor.mat.ref) <- c('Score', 'PGIS', paste0('Val_', 1:(n.val-1)))

# Create Correlated Validators:
val <- matrix( rnorm(n = n.val*length(bl), mean = mean(bl), sd = sd(bl)), nrow = length(bl), ncol = n.val)
val <- cbind(bl, val) %*% chol(cor.mat.ref)
val <- val[ , -1] # drop the Y_comp at baseline
val <- as.data.frame(val)
#cor(cbind(dat$Y_comp[dat$Time == tt], val))# confirmed
#colnames(val) <- c('PGIS', paste0('Val_', 1:(n.val-1)))
