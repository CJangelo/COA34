...
# Turn this into a Vignette
# Putting it all together
# make clear that you're passing IRT item parameters
...
# Kind of goofy, the way this function handles a scalar value...eh
  ...
  sim <- COA34::sim_dat(N = 1000, number.groups = 2, number.timepoints = 2,
...
ir <- COA34::sim_irt_item(dat = dat2, J = j, K = k,
                          latent.variable = 'Y_comp', time.var = 'Time')

ir_NA <- COA34::sim_irt_item(dat = dat3, J = j, K = k,
...
str(dat7)
table(dat7$sum_score_delta)
table(dat7$Val_1_delta)
...
#
#png(file = 'Example_eCDF_shell.png', units="in", width=11, height=8.5, res=300)
png(file = 'Example_eCDF_shell.png', units="in", width=8.5, height=5.5, res=300)

  ecdf

dev.off()

# Do ePDF as well - note that this should really be
# a barplot
