...
# --------------------------------------------------------
# MMRM
 mod.gls <- gls(Y_mar_delta ~  anchor.groups*Time,
  ...
  em <- emmeans(mod.gls, ~ anchor.groups | Time, mode = 'df.error')
  em <- as.data.frame(em)
  thr.mmrm <- em[em$Time == 'Time_4' & em$anchor.groups == 'Deteriorated_1', 'emmean']

#TODO: Show improvement of MMRM
# 1. Plot 1: Plot everything
# 2. Plot 2: Restrict plot to ONLY the deteriorate 1 category group
# include the 1 category deterioration median threshold
# (1 anchor group, 1 threshold)
# Plot 3: plot the Y_comp deteriorate 1 category
# plot the Y_mar deteriorate 1 category
# plot the 1 category deterioration median threshold
# Plot 4: same as plot 3 only include the MMRM threshold
# POINT IS TO SHOW THAT THE MMRM RECOVERS THE CORRECT THRESHOLD!
...
dat4 <- dat[t4, c('USUBJID', 'Y_mar')]
colnames(dat4) <- c('USUBJID', 'Y_mar_4')
...
xlim.max <- round(max(abs(dat.plot$Y_mar), na.rm = T))
...
#--OR--
plot.groups <- c("Deteriorated_1")#which anchor groups to include
...
# Plot:
ggplot(dat.plot, aes(x=Y_mar_bl, y=Y_mar_4, fill = anchor.groups)) +
  theme_minimal() +
